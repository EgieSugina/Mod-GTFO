...
- Stamina Mod
  - Potentially fixed a rare bug that would cause Stamina Mod to always fully drain stamina when applied until reboot.
...
- Hold Burst
  - Added field "CancelConsumeClip": Transfers the rest of the burst from the mag to the reserves when canceled.
    - Useful to ensure whole bursts are consumed, particularly if using Hold Burst to allow burst canceling.
- Trigger Holder
  - Added field "ThresholdOffset": Starts the threshold with progress on drop.
    - Useful if you want multiple separate triggers to happen in a cyclic/rotating order.
...
- General
  - Fixed an oversight with the previous melee weapon fix not being able to sync triggers with other clients.
...
- General
  - Fixed checkpoints causing weapons to be able to hit their user.
  - Fixed melee weapons not being able to sync triggers with other clients.
- Damage Over Time
  - Fixed damage over time breaking in some rare cases, failing to work until re-drop.
- Health Trigger
  - Fixed incoming damage and healing on Host not always activating Health triggers.
- Silence
  - Fixed weapons becoming completely silent in some cases despite having alert radii.
- Temp Properties
  - Fixed an error that occurred when Override was enabled and certain other properties existed on the weapon.
...
- Ammo Mod, Data Swap
  - Fixed an error that could occur in some cases when other players had a weapon with these properties.
...
- Action Lock
  - Added support for melees.
  - Separated swap and sprint locks.
- Projectile
  - Added new fields:
    - "HitDestroyDamageType": Destroys the projectile after dealing the specified damage type.
      - "Invalid" by default to disable the field.
    - "HitDestroyDelay": Delay on HitDestroy when it occurs the first time.
- Misc
  - Updated for GTfXP v2.6.0 compatibility.

**Bug Fixes**

- General
  - Fixed an incorrect clamp calculation used in multiple locations, e.g. Explosions.

## v4.11.1

**Bug Fixes**

- Damage Over Time, Explosion, Shrapnel
  - Fixed Snatchers not staggering from these effects and not releasing victims when killed
...
- Hold Burst
  - Added field "RequireHold": Require the trigger to be held to continue the burst.
    - Useful if you only need the sprint/swap cancel behavior.
- Noise
  - Removed field "LocalSoundOnly" and replaced with "LocalSoundID"
  - "SoundID" now only plays for other players, while "LocalSoundID" plays for you. "SoundID" plays for you if "LocalSoundID" is 0.
    - This lets you play 2D and 3D audio without overlapping.
  - "LocalSoundOnly" is still supported for backwards compatibility.
- Shrapnel
  - Added field "Offsets": Modifies the direction of shrapnel by angular offsets.
  - Hit-less triggers and "ApplyPositionMode": "User" now fire toward where the player is facing.
  - Using "FireFrom": "HitPos" and "ApplyPositionMode": "User" together now originates shrapnel FX from the weapon.
- Reference Trigger
  - Can now directly hold properties inside it, instead of requiring references only.
    - Useful to batch properties under a single trigger.

**Bug Fixes**

- Projectile
  - Fixed shot information pulling from an incorrect shot in some cases, mainly with Multi Shot.
- Stagger Trigger
  - Fixed only occurring once per enemy.
...
- Data Swap
  - Fixed the crosshair appearing if activated while aiming down sights.
- Armor Mod, Armor Shred, Shot Mod Debuff
  - Fixed live edit causing modifiers to persist until exiting the level.
...
- Reference Trigger
  - Added new fields:
    - "SendToTrigger": Sends applied triggers to the triggers of the referenced properties instead of activating them.
    - "SendReset": Resets the referenced properties when this propertyt resets.
      - Previously always true.
    - "LoopDuration": Time over which triggers are applied repeatedly.
    - "LoopInterval": Time between each loop iteration.
    - "LoopDelay": Time before the loop starts iterating.
    - "LoopAddNewTriggers": Allows new triggers to be added to the list of applied triggers while looping.
  - These are much-needed features to automate triggers and combine the effect of multiple reference triggers (to some degree).
- Shrapnel
  - Renamed "DamageLocks" to "DamageObjects", now also controls the ability to hit mines.
- Triggers
  - Added new field "AllowOnce": After applying, prevents any activations until the trigger is reset.

**Bug Fixes**

- Armor Mod
  - Fixed allowing only hit triggers as accepted trigger types.
- Jump Trigger
  - Fixed occurring twice.
...
- Added new Effect:
  - Action Lock: Prevents performing certain actions while active (and equipped).
- Ammo Regen
  - Added field "ChangeAmount": The minimum amount of bullets that must accrue before being given.
- Armor Mod
  - Added field "GlobalID": If non-zero, all players share the same modifier rather than creating separate modifiers.
- Armor Mod, Speed Mod, Stamina Mod
  - Added field "ApplyToTarget": Applies the modifier to the target hit rather than the user.
- Shrapnel
  - Updated client FX logic to better match FireFrom behavior.
  - Added Search fields:
    - "TriggerScaleCount": When enabled, scales shrapnel count by trigger activations. Otherwise, scales damage.
    - "SearchEnabled": Enables auto aiming shrapnel at nearby enemies.
    - "SearchCap": The maximum number of shrapnel that can be auto aimed.
    - "SearchCapPerTarget": The maximum number of shrapnel that can be auto aimed for each individual target.
    - "ScaleSearchCapPerTarget": When TriggerScaleCount is enabled, allows SearchCapPerTarget to be modified.
    - "SearchOverflow": Allows firing leftover shrapnel that Search did not use.
    - "SearchRange": Distance within which Search can acquire enemies.
    - "SearchAngle": Angle within which Search can acquire enemies.
    - "SearchShotMaxAngle": Spread applied to shots fired at Search targets.
    - "SearchTargetMode": Determines what Search shots aim for.
    - "SearchTargetPriority": Determines which enemies Search prioritizes first.
- Triggers
  - Reference: Now accepts "ReferenceID" as a valid format.
    - Useful to avoid collisions with other Trigger names.

**Bug Fixes**

- Explosive, Fire Shot, Projectile, Shrapnel, Projectile
  - Fixed DamageFriendly and DamageOwner fields not functioning correctly for bots and sentries.
- Hold Burst
  - Fixed sprint canceling fire w/ RunAndGun.
- Kill & Stagger Triggers
  - Fixed not having access to backstab fields.
- Projectile
  - Fixed homing using WallPierce's RequireOpenPath incorrectly.
- Shrapnel
  - Fixed Traits not being able to be referenced.
  - Fixed Traits not being able to use reference properties.
  - Fixed WallHandling set to Ricochet resetting PierceLimit on bounce.
  - Fixed disabling shot FX if Projectile was present on the weapon rather than in Traits.
- Stack Mods Per Target (e.g. ShotModPerTarget)
  - Fixed simultaneous triggers (e.g. when using a separate Apply or multiple Activates) not using StackType correctly.
- Triggers
  - Fixed ConsumeThreshold decreasing accrued activations immediately when reached even if Apply had separate triggers.
  - Fixed Threshold resetting before applications when using ResetPreviousOnly.
- Misc
  - Added API support to fix [FireRateFPSFix](https://thunderstore.io/c/gtfo/p/Dinorush/FireRateFPSFix/) behavior with ForceFire.
...
- Damage Over Time
  - Fixed HitmarkerCooldown hiding kill hitmarkers.
...
- Damage Over Time
  - Added field "HitmarkerCooldown": Sets a cooldown on how frequently DOT can generate hitmarkers per target.
    - Incompatible with the Trait. A simple way to get the same effect only on DOT, which was the main reason to use it anyway.

**Bug Fixes**

- Hit Taken & Damage Taken Triggers
  - Fixed damage type restrictions not functioning.
...
- Projectile
  - Can now detonate tripmines.
- Hit Taken & Damage Taken Triggers
  - Can now restrict to damage types by specifying them in the name.
    - See [wiki page](https://github.com/Dinorush/ExtraWeaponCustomization/wiki/Trigger#player-damage-type-triggers) for the full list.

**Bug Fixes**

- Enforce Fire Rate
  - Fixed firing slightly more than it should when used in tandem with [FireRateFPSFix](https://thunderstore.io/c/gtfo/p/Dinorush/FireRateFPSFix/).
- Hold Burst
  - Fixed sprinting not always canceling a burst.
- Projectile
  - Fixed DestroyedSoundID causing projectiles to despawn and behave incorrectly in some cases.
  - Reduced bounce distance inconsistency when using RicochetSpeedMod.
...
- Hold Burst
  - Bursts can now also be canceled by swapping or sprinting.

**Bug Fixes**

- Accelerate
  - Fixed DecelTime and DecelDelay having incorrect behavior when used with Hold Burst.
...
- Charge Speed Mod
  - Fixed modifiers permanently stacking in rare cases.
- Projectile
  - Fixed RicochetSpeedMod not syncing to other clients.
...
- Added new effects:
  - Charge Speed Mod: Modifies charge time (player guns only).
  - Force Fire: Forces the gun to fire (player guns only).
- Projectile
  - Added new field "MaxRange": Distance in meters that the projectile travels before disappearing.
  - Added new callback "TimeOut": Triggers when the projectile despawns from its Lifetime or MaxRange.
- Trigger
  - Added new triggers:
    - "Charge Start": Triggers when gun/melee charge starts.
    - "Charge End": Triggers when gun/melee charge ends, applying activations based on the charge amount.

**Bug Fixes**

- General
  - Fixed melee damage boosters not affecting EWC damage.
- Jump End Trigger
  - Fixed triggering in some cases when Double Jumping in the air.
- Stamina Mod
  - Fixed it failing to function at all.

## v4.7.1

**Bug Fixes**

- Fixed incompatibilities with [level-spawned sentries](https://thunderstore.io/c/gtfo/p/Inas07/EOSExt_LevelSpawnedSentry/)
...
- Trigger
  - Added new triggers:
    - "Key" | "Key Down": Triggers when the specified key is pressed.
      - E.g. "KeyConfig1", "KeyV"
    - "Key Up": Triggers when the specified key is released.
      - E.g. "KeyUpConfig2", "KeyUpB"
- Added 4 keybinds to the Config file that the above triggers can reference to allow custom player keybinds.
...
- Hit Check Cooldown
  - Fixed custom damage (e.g. Damage Over Time) causing hitmarkers to not appear as often as they should.
...
- Push
  - Now resets the falling teleport timer when applied.
  - Now resets fall damage when push goes upwards.

**Bug Fixes**

- Push
  - Fixed IncludeVelocity in cap fields causing velocity to compound on existing velocity.
...
- Projectile
  - Added new fields:
    - "RicochetIgnorePlayers": Disables owner and friendly player collisions after ricocheting.
    - "RicochetResetHits": Resets tracked hits after each ricochet.
      - Allows the projectile to damage previously hit enemies again.
...
- Shrapnel
  - Added HitPos as an accepted FireFrom value.

**Bug Fixes**

- Shrapnel
  - Fixed Projectile shrapnel causing unity logs/warnings
- Sync Trigger
  - Fixed only per-target mods being accepted as valid
...
- General
  - Fixed C-Foam causing errors when hitting an enemy

## v4.6.2

- Exposed DataAPI for other plugins to add custom data.
...
- Init Trigger
  - Added support to specify "Host" or "Client" by adding one to the trigger name.

## v4.6.0

**Features and Adjustments**

- Added new effect "Push": Pushes the user in a direction.

- Explosive
  - Added field "HitFromExplosionPos": Changes hit direction to use the target's position relative to the explosion.
- Projectile
  - Added field "ItemID": Specifies an item to fire, rather than a projectile.

**Bug Fixes**

- General
  - Fixed Pouncers causing errors when touching C-Foam
  - Fixed IDs in property fields not reading PartialData string IDs
...
- Projectile
  - Fixed projectiles hitting through walls
  - Greatly reduced instances where ricochet projectiles could bounce through walls, especially with low HitCheckCooldown

## v4.5.5

**Bug Fixes**

- PreSwing and Swing Triggers
  - Fixed occurring twice
...
- Added new triggers:
  - "PrePush": Triggers at the start of a push.
  - "Push": Triggers at the end of a push.
  - "PushHit": Triggers when push hits a pushable enemy.

**Bug Fixes**

- General
  - Fixed incorrect sentry trigger networking causing client errors 
- Fire Shot
  - Fixed ArchetypeID not applying
...
- Projectile
  - Added field "HitCheckCooldown": Interval in seconds between hit detection checks.
    - Can be raised to improve performance, especially with high-pellet count shotguns.

**Bug Fixes**

- Enforce Fire Rate
  - Fixed a case where, when added/removed with Temp Properties, it would fire significantly more than it should.

## v4.5.2

- Update compatibility with renamed ModifierAPI

## v4.5.1

- Update compatibility with GtfXP v2.4.12

## v4.5.0

**Features and Adjustments**

- Removed [Gun] restriction from the properties:
  - Fire Shot
  - Shrapnel
  - Spread Mod
  - Pierce Multi
  ...
  - Thick Bullet
  - Wall Pierce
  - All of the above shot-modifying properties only work on Fire Shot/Shrapnel, except Pierce Multi
- Added support for level-spawned sentries

## v4.4.3

**Features and Adjustments**

- Added new trigger "ModSync": On a sub-trigger, applies an amount matching the referenced Mod's stacks.
  - E.g. On "Reload", apply an amount equal to the stacks on the "Shot Mod" effect with ID "Growth".
...
- Data Swap
  - Fixed swapped archetype/audio being preserved over checkpoints
...
- General
  - Fixed some rare cases where client vanilla tracers were over-canceled and caused errors

## v4.4.0

**Features and Adjustments**

- Added new effect "InfectionMod": Modifies the user's infection.

**Bug Fixes**

- Health Mod
  - Fixed EnableDamageFX not disabling.
- Staggered Damage Type
  - Fixed the damage type being read as the "Stagger" trigger when attached to "Kill" or "Damage" triggers.
...
- Per Target Trigger
  - Added new fields:
    - "ActivateScaleWithFalloff": Scales stored activation amounts by their falloff.
    - "ApplyAboveThreshold": Reduces stored amount by Threshold when applied.
    - "OverrideAmount": Forces the total applied amount to be Amount.

**Bug Fixes**

- Per Target Trigger
  - Fixed threshold not working in some cases
...
- Damage Types
  - Added damage types "Staggered" and "Unstaggered".
...
- Reset Trigger
  - Fixed stored activations not being cleared on Apply if Delay > 0
...
- Explosive, Shrapnel
  - Added field "ApplyPositionMode": Determines where the effect occurs when using positional triggers.
    - "ApplyOnUser" has been merged into this (sets it to "User" mode).
- Health Mod
  - Added field "EnableDamageFX": If heal is negative, applies flinching and screen flash.

**Bug Fixes**

- Health Mod
  - Fixed negative heals causing errors when using ApplyToTarget

## v4.2.7

**Features and Adjustments**

- Added new Effect "StaminaMod": Grants or takes away stamina from the user when triggered.
- Hit Triggers
  - Added Per Target as a valid trigger.
- Per Target Trigger
  - Added field "Cancel": The trigger that resets stored effect activations (for that target).
  - Added field "Threshold": The minimum Amount that must be accrued before Apply goes through.
  - Accruing any Amount now starts ResetDelay.
...
- Auto Aim
  - Fixed aim check incompatibility with Aim Toggle
...
- Projectile
  - Added new fields:
    - "FlyingSoundID": The sound to play when spawned, following the projectile.
    - "DestroyedSoundID": The sound to play when destroyed.
    - "StopFlyingSoundOnDestroy": Stops the flying sound when destroyed.
...
- General
  - Fixed piercing shotguns doing double damage if PierceBugFix was not installed
...
- Kill Trigger
  - Fixed a minor case where kill triggers could fire multiple times on a dead enemy
...
- General
  - Fixed Sentries causing errors when doing reset triggers across the network
...
- Reload Mod
  - Inverted mod application so that 2 mod halves reload time.
  - (This exists by the way)
- Stack Mods (Shot Mod, Recoil Mod, etc.)
  - Renamed field "OverrideStackType" to "InnerStackType".
  - "InnerStackType" now also applies to Max/Min.

**Bug Fixes**

- General
  - Fixed burst-related logic causing errors
- Per Target Trigger
  - Fixed shorthand notation ("Per[X]On[Y]") not working
...
- Auto Aim
  - "Angle" of 0 can now acquire targets when directly looking at them.
  - Added new fields:
    - "RequireLOS": Requires line of sight to obtain a lock.
    - "StayRequireLOS": Requires line of sight to maintain a lock when StayOnTarget is enabled.
    - "StayAngle": Used instead of Angle to maintain a lock when StayOnTarget is enabled.
    - "StayRange": Used instead of Range to maintain a lock when StayOnTarget is enabled.
- Projectile
  - Added field "DirChanges": A list of objects that allow changing the projectile's direction over time.
    - Kind of jank, but it's already coded, oh well.
- Spread Mod
  - Added field "UpdateCrosshair": Updates the crosshair when applied/removed.
    - Always enabled previously.

**Bug Fixes**

- General
  - Fixed bullet FX going through enemies in some cases where it shouldn't
- Fire Shot, Multi Shot
  - Fixed X offsets going in the opposite direction
...
- Misc
  - Fixed internal logic ignoring broken limbs that could still be damaged
...
- Misc
  - Fixed certain properties breaking on checkpoint reload/re-dropping into a level (again)
- Shrapnel
  - Fixed MaxAngle < 180 having 0 spread if placed on a floor or ceiling
...
- Armor Shred
  - **Breaking change**: "Inverted" effect from Mod = 0 fully shredding armor to Mod = 2 fully shredding armor.
  - Now supports StackLayer.
  - If Armor Shreds combine to a Mod > 2, damage to armor is increased linearly (e.g. Mod = 3 gives 2x damage).
- Armor Pierce
  - Added new "Type" as default: "OverPierce": Functions identically to Armor Shred.
- Explosive, Shrapnel
  - Added field "ApplyOnUser": Forces positional triggers to occur at the user.
- Fire Rate Mod
  - "ForceUpdate" now also applies on reset triggers.
- Projectile
  - Added HomingSettings field "UseSteadyStrength": Changes homing strength logic to be consistent instead of depending on the turn amount left.
    - Normally, it turns faster if it needs to turn more, following Vanilla logic. This field makes it turn at a constant rate.
- Triggers
  - Hit-based Triggers
    - Added new fields:
      - "MinBackstabRequired" and "MaxBackstabRequired": The portion of backstab bonus needed for the trigger to occur.
  - Added new Trigger: 
    - "Backstab": A damage type trigger that scales with backstab bonus.

**Bug Fixes**

- General
  - Fixed melee weapons not applying live edit changes
  - Fixed checkpoints bricking weapons
- Armor Shred
  - Fixed not applying to Explosive/DoT/Shrapnel/Foam/Vanilla weapons
- Multi Shot
  - Fixed not doing any friendly fire
- Projectile
  - Fixed "Weakspot" mode homing not functioning at all
  - Fixed projectile sentries benefitting from the host's gun damage modifier from GtfXP
- Shrapnel
  - Fixed not registering damage to DamageSync
...
- Misc
  - Fixed clients sometimes having errors occur on drop
...
- Misc
  - Fixed non-custom weapons applying booster damage effects twice
...
- Misc
  - Fixed player/bot guns not computing XP/booster modifiers, preserving modifiers from melee or sentry guns
  - Fixed miss triggers erroneously activating after firing a non-custom gun/sentry/melee
...
- Misc
  - Fixed pierce weapons using incorrect distance between pierce hits and hitting in the wrong order (for real this time)
...
- Multi Shot
  - Fixed dealing no direct damage

## v4.0.0

### Bot and Sentry Support!

**Features and Adjustments**

- Added Bot and Sentry support
  - Most things are supported, but certain triggers and properties are restricted.
  - Bot melees are not supported.
- Base Fields
  - Added "IgnoreHeldSentry": Does not apply the properties to the sentry while it is on the player.
- Ammo Mod, Ammo Regen
  - Added field "BypassClipCap" - ClipChange can exceed the maximum clip.
    - Mainly useful for Sentries.
- Fire Rate Mod
  - Now affects melee animation speeds in addition to charge speed.
- Projectile
  - Added property triggers:
    - "Alive" - Occurs periodically while the projectile is flying.
    - "Destroyed" - Occurs when the projectile dies.
  - Added fields:
    - "AliveTriggerDelay": Seconds before "Alive" trigger begins occurring.
    - "AliveTriggerInternal": Seconds between each "Alive" trigger.

**Bug Fixes**

- General
  - Explosive/DoT/Shrapnel now benefit from boosters and melee syringe buffs

## v3.7.11

**Bug Fixes**

- Fixed "0 cannot be greater than -1" errors that occurred with certain tiles
  - Now depends on AmorLib, which fixed the relevant code and lifted it to a public API for multiple mods to share

## v3.7.10

**Bug Fixes**

- Armor Shred Debuff, Shot Mod Debuff
  - Now properly applies to sentry and bot damage
...
- Trigger
  - Added holder field "CancelApplyDelay": When a Cancel occurs, clears any delayed applications.

**Bug Fixes**

- Temp Properties
  - Fixed a case where if Override is enabled and another TempProperties is referenced and active, it fails to enable that TempProperties' held properties
...
- Ammo Mod
  - Renamed field "OverflowToReserve" to "OverflowAtBounds".
  - Negative "ReserveChange" now underflows to the magazine when "OverflowAtBounds" is enabled.
- Noise
  - Added field "FollowUser": Causes the sound to follow the user, instead of being suspended.
...
- Explosive
  - Now applies force to enemies using the relative direction, causing them to stagger away from the explosion.
    - Previously always used straight up, which is ambiguous.
- Explosive & Shrapnel
  - Added field "ApplyAttackCooldown": Causes enemies to suffer damage taken cooldowns on their abilities.
    - Same as the field on Damage Over Time.

**Bug Fixes**

- Misc
  - Fixed search algorithms causing errors and log spam in some scenarios

## v3.7.6

**Bug Fixes**

- Clip & Health Triggers
  - Fixed 0 amount logic working incorrectly in several cases
- Misc
  - Exposed API to fix FireRateFPSFix compatibility issues
...
- Projectile
  - Added field "EnableTerrainHitFX": Shows graphics when hitting terrain (bullet holes, dust, and sound).
- Clip & Health Triggers
  - 0 amount now only applies on effects with StackType Override/None.
- Misc
  - Improved runtime of search algorithms at doorways.

**Bug Fixes**

- Projectile
  - Fixed incorrect logic with RicochetSpeedMod that caused only RicochetSpeedAngleFactor to apply
...
- Clip & Health Trigger
  - Now only apply if ammo/health actually changes
  - 0 amount now works on Activate triggers to allow for easy zeroing-out of effects
